...
public class Admin_InsDoc extends JFrame {
	 ...
	 * Create the frame.
	 */
	public Admin_InsDoc(MySQLConnection connection, String userDni) {
		...
		JLabel lblHint = new JLabel("Añadir nuevo doctor");
		...
		bornLbl.setBounds(581, 347, 541, 32);
		...
		MainButton insertBtn = new MainButton("Añadir doctor");
		insertBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				insertDoctor();
			}
		});
		insertBtn.setBounds(401, 463, 310, 44);
	...
	private boolean checkFields() {
		if (dniField.getText().isEmpty()) {
			...
			JOptionPane.showMessageDialog(this, "La contraseña debe tener una longitud mayor a 8 y menor a 16.", "Error",
					JOptionPane.ERROR_MESSAGE);
			return false;
		} else if (String.valueOf(repeatPassField.getPassword()).isEmpty()) {
			...
			JOptionPane.showMessageDialog(this, "La contraseña no coincide.", "Error",
					JOptionPane.ERROR_MESSAGE);
			return false;
	...
	// efectúa la inserción del doctor en función de los campos rellenos
	private void insertDoctor() {
		if (checkFields()) {
			...
			try {
				if (mailField.getText().isEmpty() && 
						((dayCb.isEnabled() && dayCb.getSelectedItem().equals("")) ||
								!dayCb.isEnabled())) {
					affectedRows = controller.insertNewDoctor(dni, password, nombre, apellidos, telefono);
				} else if ((dayCb.isEnabled() && dayCb.getSelectedItem().equals("")) ||
								!dayCb.isEnabled()) {
					String correo = mailField.getText();
					affectedRows = controller.insertNewDoctor(dni, password, nombre, apellidos, telefono, correo);
					...
					affectedRows = controller.insertNewDoctor(dni, password, nombre, apellidos, fecha_nacimiento, telefono);
				} else {
					String correo = mailField.getText();
					String fecha_nacimiento = String.valueOf(yearCb.getSelectedItem())
							+"-"+String.valueOf(monthCb.getSelectedItem())
							+"-"+String.valueOf(dayCb.getSelectedItem());
					affectedRows = controller.insertNewDoctor(dni, password, nombre, apellidos, telefono, fecha_nacimiento, correo);
				}
			...
			if (affectedRows == 0) {
				JOptionPane.showMessageDialog(this, "No se ha podido añadir al doctor. Es posible que el DNI ya esté en uso.", "Error",
						JOptionPane.ERROR_MESSAGE);
			} else {
				JOptionPane.showMessageDialog(this, "El doctor ha sido incorporado con éxito.", "Éxito",
