...
public class Admin_ConsDoc extends JFrame {
	 ...
	 * Create the frame.
	 */
	public Admin_ConsDoc(MySQLConnection connection, String userDni) {
		...
		JLabel lblHint = new JLabel("Panel de doctores");
		...
		String[] colNames = {"DNI", "Nombre", "Apellidos", "Teléfono", "Fecha de nacimiento", "Correo electrónico", "Baja"};
	    ...
	    JMenuItem menuItemModify = new JMenuItem("Modificar doctor");
	    menuItemModify.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String[] rowData = Menu_Controller.getSelectedTableRowData(table);
				Menu_Controller.openModDoc(connection, userDni, rowData);
				dispose();
			}
		});
	    
	    JMenuItem menuItemCheckAppts = new JMenuItem("Consultar citas");
	    menuItemCheckAppts.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String[] rowData = Menu_Controller.getSelectedTableRowData(table);
	    ...
	    JMenu menuSpecialty = new JMenu("Especialidades");
	    JMenuItem menuItemAddSpec = new JMenuItem("Añadir especialidad");
	    menuItemAddSpec.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String[] rowData = Menu_Controller.getSelectedTableRowData(table);
				assignSpecialty(rowData[0]);
			}
		});
	    JMenuItem menuItemConsSpec = new JMenuItem("Consultar especialidades");
	    menuItemConsSpec.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String[] rowData = Menu_Controller.getSelectedTableRowData(table);
				Menu_Controller.openSpecialties(connection, userDni, rowData);
				dispose();
			}
		});
	    
	    menuSpecialty.add(menuItemAddSpec);
	    menuSpecialty.add(menuItemConsSpec);
	    popupMenu.add(menuItemModify);
	    popupMenu.add(menuItemCheckAppts);
	    popupMenu.add(menuSpecialty);
	    ...
	    // mostramos todos los doctores en la tabla
	 	listDoctors();
		
		MainButton listBtn = new MainButton("Mostrar doctores");
		...
		MainButton findBtn = new MainButton("Buscar un doctor");
		...
		MainButton insertarBtn = new MainButton("Añadir nuevo doctor");
		insertarBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				Menu_Controller.openInsDoc(connection, userDni);
				dispose();
			}
		});
	...
	// obtiene todos los doctores y los añade a la tabla
	private void listDoctors() {
		...
		controller.listDoctors(currentModel);
	}
	
	// obtiene el doctor indicado y lo añade a la tabla vaciándola previamente
	private void findDoctor() {
		String input = JOptionPane.showInputDialog(
				   this,
				   "Introduzca el DNI del doctor:",
				   "Buscar un doctor",
		...
		if (input != null && Menu_Controller.checkDni(input)) {
			Doctor_Controller controller = new Doctor_Controller(connection);
			DefaultTableModel currentModel = (DefaultTableModel) table.getModel();
			controller.findDoctor(currentModel,input);
			
			if (currentModel.getRowCount() == 0) {
				JOptionPane.showMessageDialog(this, "No se ha encontrado ningún doctor con este DNI.", "Sin resultados",
	...
	// permite consultar las citas del doctor seleccionado
	private void checkAppointments(String[] rowData) {
        ...
        if (choice == 0) {
        	Menu_Controller.openDocAppointments(connection, userDni, rowData, null);
        	dispose();
        } else if (choice == 1) {
        	String input = JOptionPane.showInputDialog(
 				   this,
	 		...
	 		if (input != null && !input.isEmpty()) {
	 			Menu_Controller.openDocAppointments(connection, userDni, rowData, input);
	        	dispose();
	 		} else if (input != null){
	...
	// permite asignar una especialidad al doctor cuyo DNI ha sido pasado por parámetro
	private void assignSpecialty(String dni) {
		Especialidad_Controller spec_controller = new Especialidad_Controller(connection);
		ArrayList<Especialidad> specialties = new ArrayList<Especialidad>();
		
		try {
			// obtenemos todas las especialidades disponibles
			specialties = spec_controller.getEspecialidades();
			// obtenemos todas las especialidades del doctor
			ArrayList<Especialidad> doc_specialties = spec_controller.getEspecialidades(dni);
			
			// recorremos la lista con todas las especialidades y eliminamos las que ya tiene asignadas
			// el doctor
			Iterator<Especialidad> iterator = specialties.iterator();
			while (iterator.hasNext()) {
			    Especialidad spec = iterator.next();
			    
			    boolean found = false;
			    for (int i = 0; i < doc_specialties.size() && !found; i++) {
			    	if (spec.getId() == doc_specialties.get(i).getId()) {
				        iterator.remove();
				        found = true;
				    }
				}
		...
		if (!specialties.isEmpty()) {
			String[] choices = new String[specialties.size()];
			
			for (int i = 0; i < specialties.size(); i++) {
				choices[i] = specialties.get(i).getNombre();
			}

		    String input = (String) JOptionPane.showInputDialog(this, "Seleccione qué especialidad desea asignar al doctor:",
		        "Añadir especialidad", JOptionPane.QUESTION_MESSAGE, null,
		        choices,
		        choices[0]);
		    ...
		    if (input!=null) {
				...
				try {
					affectedRows = doc_controller.assignSpecialty(dni, input);
				} catch (SQLException e) {
					System.out.println(e.getMessage());
				}
				
				if (affectedRows == 0) {
					JOptionPane.showMessageDialog(this, "No se ha podido asignar la especialidad al doctor.", "Error",
							JOptionPane.ERROR_MESSAGE);
				} else {
					JOptionPane.showMessageDialog(this, "La especialidad ha sido asignada con éxito.", "Éxito",
							JOptionPane.INFORMATION_MESSAGE);
				}
		    }
		} else {
			JOptionPane.showMessageDialog(this, "No hay especialidades disponibles. Es posible que el doctor ya tenga todas asignadas.", "Especialidades no disponibles",
					JOptionPane.WARNING_MESSAGE);
		}
	}
}
